...
Figure 1 (study design) is drawn by hand and has no script.
...
| `SourceData_Figure2_Years.csv` | Contrast by publication year; Results 3.2 |
...
| `SourceData_Figure3_AreaYear.csv` | Research area × publication year cells; Figure 3 |
| `SourceData_Figure3_metrics.csv` | Three network summaries; Figure 4c |
...
| `SourceData_Figure4_DistanceEnrichment.csv` | Enrichment by title-content distance; Figure 4a |
...
| `SourceData_ED3_network_edges.csv` | Standardized 32 × 32 citation-flow matrices for both journal groups; Figure 2c, Figure 4b |
| `SourceData_ED3_lodo.csv`, `SourceData_ED3_sensitivities.csv` | Leave-one-area-out, winsorized, and regenerated-fit estimates; Figure 4c, Table S3 |
| `SourceData_ED4_nodes.csv` | Node positions on the title-content map; Figure 4b |
...
| `SourceData_Hierarchy_Areas.csv` | Display names of the 31 research areas |
...
| `source_data_manifest.csv` | SHA-256 and row count of every `SourceData_*.csv`, checked by the manuscript's validator |
